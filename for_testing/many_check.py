...
def send_req(peoples):
    ...
    for people in peoples:
        ...
        try:
            ...
            result = None
        # Проверить, что пришел результат
        if result:
            # В нем обязательно должен быть тег Status = Result
            if result.find('<smev:Status>RESULT</smev:Status>') > 0:
                err = 0
        total_err += err
    con.close()
    return total_err
...
if __name__ == '__main__':
    ...
    for thread in (1, 3, 5): #, 7, 10, 12, 15, 20 , 25):
        ...
        err_list = pool.map(send_req, for_test)
        pool.close()
        pool.join()
        stop = time.time()
        for err in err_list:
